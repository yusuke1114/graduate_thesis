...
シミュレーションの結果、MLトレーダーはZITの行動予測について正解率99％で予測することができた。また、その結果を図4.1に混同行列として示す。
...
図4.2にZITだけでシミュレーションした時の最終資産分布の相補累積を示す。ガウス分布通りの結果であることがわかった。
図4.3~7に、MLトレーダーの市場混入率を10％~50％まで変化させた際の、最終資産分布の相補累積を示す。
これより、10％、20％においてファットテールがが出現しており、それ以降ではガウス分布に従っていることが分かった。

\section{ML TraderとRule Traderの比較}
図4.8~12に、Ruleトレーダーの市場混入率を10％~50％まで変化させた際の、最終資産分布の相補累積を示す。
...
これは、市場内の資金がZITからMLへと一方的に移転していることを示している。

\subsection{トレード履歴}
表4.1~5にZITとMLトレーダーの市場においての行動取引統計を示す。これから、MLトレーダーは高い値段で買い、安い値段で売っているが、売りの回数が買いの回数よりも多いため、利益を得ていることが分かった。
また、表4.6~10にZITとRuleトレーダーの市場においての行動取引統計を示す。これから、Ruleトレーダーは高い値段で買い、安い値段で売っているが、MLトレーダーよりも売りと買いの回数の差が少ないために損をしていることが分かった。
また、図4.13～17にZITとMLトレーダー市場での行動の割合の画像を示す。ここから、MLトレーダーはほぼ不成立がないことがわかった。

\begin{table}[htbp]
    \centering
    \caption{ZITとMLトレーダーの行動別取引統計（混入率10\%のケース）}
    ...
    \begin{tabular}{lllrrr}
        ...
        ML  & Buyer & Overwrite & 4,285 & 604,310 & 141.0 \\
            ...
            \cline{2-6}
            & Seller& Overwrite & 8,746 & 695,425 & \textbf{79.5} \\
            &       & Executed  & 10,765 & 928,609 & \textbf{86.3} \\
        \hline
    \end{tabular}
\end{table}
...
\begin{table}[htbp]
    \centering
    \caption{ZITとMLトレーダーの行動別取引統計（混入率20\%のケース）}
    ...
    \begin{tabular}{lllrrr}
        ...
        ML  & Buyer & Overwrite & 8,282 & 1,153,394 & 139.3 \\
            ...
            \cline{2-6}
            & Seller& Overwrite & 18,857 & 1,506,930 & \textbf{79.9} \\
            &       & Executed  & 21,466 & 1,813,258 & \textbf{84.5} \\
        \hline
    \end{tabular}
\end{table}
...
\begin{table}[htbp]
    \centering
    \caption{ZITとMLトレーダーの行動別取引統計（混入率30\%のケース）}
    ...
    \begin{tabular}{lllrrr}
        ...
        ML  & Buyer & Overwrite & 11,745 & 1,614,503 & 137.5 \\
            ...
            \cline{2-6}
            & Seller& Overwrite & 30,672 & 2,485,957 & \textbf{81.0} \\
            &       & Executed  & 32,325 & 2,680,963 & \textbf{82.9} \\
        \hline
    \end{tabular}
\end{table}
...
\begin{table}[htbp]
    \centering
    \caption{ZITとMLトレーダーの行動別取引統計（混入率40\%のケース）}
    ...
    \begin{tabular}{lllrrr}
        ...
        ML  & Buyer & Overwrite & 14,360 & 1,978,542 & 137.8 \\
            ...
            \cline{2-6}
            & Seller& Overwrite & 44,293 & 3,600,807 & \textbf{81.3} \\
            &       & Executed  & 43,328 & 3,502,286 & \textbf{80.8} \\
        \hline
    \end{tabular}
\end{table}
...
\begin{table}[htbp]
    \centering
    \caption{ZITとMLトレーダーの行動別取引統計（混入率50\%のケース）}
    ...
    \begin{tabular}{lllrrr}
        ...
        ML  & Buyer & Overwrite & 16,405 & 2,235,514 & 136.3 \\
            ...
            \cline{2-6}
            & Seller& Overwrite & 59,687 & 4,885,464 & \textbf{81.9} \\
            &       & Executed  & 54,411 & 4,264,689 & \textbf{78.4} \\
        \hline
    \end{tabular}
\end{table}
...
\begin{table}[htbp]
    \centering
    \caption{ZITとRuleトレーダーの行動別取引統計（混入率10\%のケース）}
...
\begin{table}[htbp]
    \centering
    \caption{ZITとRuleトレーダーの行動別取引統計（混入率20\%のケース）}
...
\begin{table}[htbp]
    \centering
    \caption{ZITとRuleトレーダーの行動別取引統計（混入率30\%のケース）}
...
\begin{table}[htbp]
    \centering
    \caption{ZITとRuleトレーダーの行動別取引統計（混入率40\%のケース）}
...
\begin{table}[htbp]
    \centering
    \caption{ZITとRuleトレーダーの行動別取引統計（混入率50\%のケース）}
...
Rule TraderとZITの市場の相補累積分布関数10～50
\begin{figure}[htbp]
    \centering
