...
つまり、深層強化学習モデルは適応的な振る舞いを再現できる一方でその内部論理がブラックボックスである。一方で、Mizutaが指摘するように、市場メカニズムの解明や制度設計への応用には、結果の説明可能性が不可欠である\cite{Mizuta}。
ZIトレーダーやGenoaモデルはメカニズムが明確であるが、状況に合わせて賢く行動を変える能力を欠いている。

\section{研究の目的}

前節で述べたように、金融市場に機械学習やアルゴリズム取引の導入は市場の効率性を向上させる一方で、市場を不安定化させる要因になり得る。
そこで本研究は単純なルールに支配されるZIトレーダー市場に、機械学習プレイヤーが参入した際の影響を調べることを目的とする。
まず、ZIトレーダーで構成される市場環境を構築し、そこへ約定確率を予測するニューラルネットワーク型の機械学習トレーダーを導入する。
この混合市場においてシミュレーションを行い、市場の統計的特性がどのように変化するかを定量的に評価する。
さらに、機械学習トレーダーが市場構造から何を学習したのかというメカニズムを明らかにするために行動を模倣した固定戦略トレーダーを導入し、比較検証を行う。


\section{本論文の構成}

    本研究では、ゼロインテリジェンストレーダーで構成される市場に機械学習トレーダーを導入し、その影響について解析を行う。以下に本論文の構成を示す。
    第\ref{chap:introduction}章では、金融市場における機械学習導入の影響と金融市場シミュレーションにおいての問題点を示した。
    第\ref{chap:genri}章では、本シミュレーションの基盤となるZIトレーダーモデル、使用したニューラルネットワークについて説明する。
...
GodeとSunderは、ZIトレーダーを制約なしモデル(ZI-U)と制約付きモデル(ZI-C)の2種類に分類している。ZI-Uは赤字となる取引も許容し、完全にランダムな価格で注文を出すもでるであり、ZI-Cは利益が負になる注文は出さないという予算制約のみを課したモデルである。本研究では、人間による市場取引の特性に近い挙動を示すことが確認されている ZI-C を採用し、以降単にZITと呼称する。
...
相補累積分布関数(Complementary Cumulative Distribution Dunction,CCDF)は確率密度を積分した形式をとるため、データの離散に伴うノイズが平滑化される。
...
まず、取引成立するケースについて述べる。板に売り注文$P_\mathrm{board}$が滞留している状態で、新たな買い注文$P_\mathrm{order}$が$P_b \ge P_\mathrm{board}$という条件を満たして提示された場合、即座に売買が成立する。買いと売りが逆のケースも同様に売買が成立する。この際、約定価格は板に待機していた注文の価格が採用される。
...
図\ref{fig:ccdf_zit}にZIトレーダーだけでシミュレーションした時の最終資産分布の相補累積分布関数を示す。ガウス分布とよく一致する分布が得られた。
図\ref{fig:cML10}から\ref{fig:cML50}に、MLトレーダーの市場混入率を10\%から50\%まで変化させた際の、最終資産分布の相補累積分布関数を示す。
...
MLまたはRBトレーダーの市場全体に対する混入率を10\%から50\%まで変化させた際、最終的な資産分布をガウス分布と仮定してフィッティングを行い、そのパラメータ（平均 $\mu$、分散 $\sigma$）の変化を計測した。
ZIトレーダーのみで市場をシミュレーションした結果、$\mu$は500であり、$\sigma$は720となった。
表\ref{tab:asset_stats_ml}にZIトレーダーとMLトレーダーの資産分布パラメータ推移を示す。
すべての混入率において、MLトレーダーの資産分布は平均840付近に集中し、ZIトレーダーの分布を大きく上回る結果となった。
市場の資金がZIトレーダーからMLトレーダーへと一方的に移転していることがわかった。また、標準偏差がすべてZIトレーダーのみでの市場よりも上回っていることがわかった。
表\ref{tab:asset_stats_rule}にZIトレーダーとRBトレーダーの資産分布パラメータ推移を示す。
市場内の資金がMLトレーダーとは異なり、ZIトレーダーへと一方的に流入していることがわかった。また、標準偏差はすべてZIトレーダーがRBトレーダーを上回っていた。
...
\subsection{トレード履歴}
表\ref{tab:trade_stats_10pct}から\ref{tab:trade_stats_50pct}にZIトレーダーとMLトレーダーの市場においての行動取引統計を示す。MLトレーダーは高い値段で買い、安い値段で売っているが、売りの回数が買いの回数よりも多いため、利益を得ていることが分かった。
また、表\ref{tab:rule_stats_10pct}から\ref{tab:rule_stats_50pct}にZIトレーダーとRBトレーダーの市場においての行動取引統計を示す。RBトレーダーは高い値段で買い、安い値段で売っているが、MLトレーダーよりも売りと買いの回数の差が少ないために損をしていることが分かった。
