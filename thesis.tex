...
\begin{figure}[htbp]
    ...
    \caption{正規分布とファット・テール分布の比較\cite{fattail}}
    \label{fig:fat}
\end{figure}
...
また、CCDFはべき乗則の検証において有効である。対象の分布がパレート分布などのべき分布に従う場合、CCDFを両対数グラフにプロットすると、その軌跡は図図\ref{fig:fat}のように右下がりの直線を描く。その傾きから、ファットテールの有無を検証することができる。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=10cm]{fig/fattail2.pdf}
    \caption{ファットテールと正規分布のCCDF}
    \label{fig:fat2}
\end{figure}
...
市場内の資金がMLトレーダーとは違い、ZIトレーダーへと一方的に流入していることがわかった。また、標準偏差はすべてZIとれーだーがRBトレーダーを上回っていた。
